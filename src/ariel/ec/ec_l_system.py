...
def mutate_one_point_lsystem(lsystem, mut_rate, add_temperature=0.5):
    ...
    if random.random() < mut_rate:
        ...
        match action:
            ...
            case "rm_rule":
                if splitted_rules[gene_to_change][:4] in [
                    ...
                    op_completed = (
                        "REMOVED : "
                        + splitted_rules[gene_to_change]
                        + " "
                        + splitted_rules[gene_to_change + 1]
                    )
        ...
        if new_rule != "":
            ...
            lsystem.rules[list(rules.keys())[rule_to_change]] = lsystem.rules[
                list(rules.keys())[rule_to_change]
            ]
    return op_completed
